...
const initUI = async () => {
  ...
  setVideoDeviceBtn.onclick = async () => {
    ...
    // alert(`You're video device has been set to: ${selectedVideoDevice.text}`);  // todo add toast
  ...
  document.getElementById('join-btn').onclick = async () => {
    ...
    await VoxeetSDK.session.open({ name: document.getElementById('name-input').value }).catch((err) => console.error(err));
  ...
  document.getElementById('stop-screenshare-btn').onclick = async () => {
    ...
    VoxeetSDK.conference.stopScreenShare().then(() => {
      //update ui
      document.getElementById('start-screenshare-btn').classList.remove('d-none');
      document.getElementById('stop-screenshare-btn').classList.add('d-none');
    })
